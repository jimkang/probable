...
var settings = {
  rangeTableAParams: [[[0, 80], 'a'], [[81, 95], 'b'], [[96, 100], 'c']],
  rangeTableBParams: {
    success: 20,
    doover: 5,
    failure: 30
...
suite('roll', function rollSuite() {
  test('should roll results that are within 0 and 5.', function rollD6(testDone) {
    for (var i = 0; i < 100; ++i) {
  ...
  test('should roll results that are within 0 and maxInt - 1.', function rollD6(testDone) {
    for (var i = 0; i < 100; ++i) {
  ...
  test('should roll 0 when rolling a 0-sided die.', function rollD6(testDone) {
    for (var i = 0; i < 100; ++i) {
  ...
  test('should roll 0 when rolling a 1-sided die.', function rollD6(testDone) {
    for (var i = 0; i < 100; ++i) {
...
suite('rollDie', function rollDieSuite() {
  test('should roll results that are within 1 and 6, inclusive.', function rollD6(testDone) {
    for (var i = 0; i < 100; ++i) {
  ...
  var maxInt = 9007199254740992;
  test('should roll results that are within 1 and maxInt, inclusive.', function rollD6(testDone) {
    for (var i = 0; i < 100; ++i) {
  ...
  test('should roll 1 when rolling a 1-sided die.', function rollD6(testDone) {
    for (var i = 0; i < 100; ++i) {
...
suite('pickFromArray', function pickFromArraySuite() {
  test('should return the emptyArrayDefault when picking from an empty array.', function pickFromEmpty(testDone) {
    assert.equal(probable.pickFromArray([], 'Empty'), 'Empty');
    testDone();
  });

  test('should return the emptyArrayDefault when picking from a non-array.', function pickFromNonArray(testDone) {
    assert.equal(probable.pickFromArray({}, 'Empty'), 'Empty');
    testDone();
  });

  test('should return the emptyArrayDefault when picking from undefined', function pickFromUndefined(testDone) {
    assert.equal(probable.pickFromArray(undefined, 'Empty'), 'Empty');
    testDone();
  });

  test(
    'should always return the same result when picking from a ' +
  ...
  test(
    'should always return an element in the array when picking from a ' +
      'two-element array',
...
suite('createRangeTableFromDict', function createRangeTableFromDictSuite() {
  test('should create a rangeTable from a dict', function createRangeTableFromDictTest(testDone) {
    var outcome;
    ...
    for (var j = 0; j <= 100; ++j) {
      outcome = table.roll();
      assert.ok(
        outcome === 'failure' || outcome === 'success' || outcome === 'doover'
      );
      // TODO: Make sure the outcome distribution is reasonable.
    }
...
suite('convertDictToRangesAndOutcomePairs', function convertSuite() {
  test('should return an array of arrays, sorted by size', function convertTest() {
...
suite('getCartesianProduct', function crossArraysSuite() {
  test('should combine every element in array A with every element in array B', function crossArraysTest() {
    var a = ['a', 'b', 'c', 'd'];
    var b = [0, 1, 2];
  ...
  test('should gemerate an empty array if one of the params is an empty array', function crossEmptyArrayTest() {
    var a = ['a', 'b', 'c', 'd'];
    var b = [];
    assert.deepEqual(probable.getCartesianProduct([a, b]), []);
  });

  test('cross array with one-element array', function crossOneElementArrayTest() {
    var a = ['a', 'b', 'c', 'd'];
    var b = [100];
    assert.deepEqual(probable.getCartesianProduct([a, b]), [
      ['a', 100],
  ...
  test('should get the Cartesian product of three arrays', function threeArrayCartesianProductTest() {
  ...
  test('should get the Cartesian product of four arrays', function fourArrayCartesianProductTest() {
    var product = probable.getCartesianProduct([
      [null, 'a', 'b'],
      ['omega', 'gamma'],
      [null, 1, 2],
...
suite('Shuffle', function shuffleSuite() {
  test('should shuffle', function shuffleTest(testDone) {
    ...
    assert.deepEqual(shuffled, [5, 1, 12, 10, 6, 7, 2, 8, 4, 3, 11, 9, 0]);
